...
func knockMultipleTimes (times : Int) {
    for i in 1...times {
        println("I knocked the number of \(i) times")
    }
}

knockMultipleTimes(6)
...
func tellNameAndAge (name : String, yearOfBirth : Int) {
    ...
    println ("\(name), my age is \(age), where \(age) is the var of course!")
}

tellNameAndAge("Kevin", 1985)

func createCarNameLabel(brand: String, model: String) -> String {
    return "\(brand) \(model)"
}

var fordFocus = createCarNameLabel("Ford", "Focus")
tellNameAndAge(fordFocus, 2000)
